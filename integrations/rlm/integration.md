...
RLM is an optional case-corpus helper. It inspects case-contained files and writes `cases/{project}/data/rlm-analysis.json`. Investigators may treat the output as leads. Fact-checking still validates claims from canonical case files, not raw RLM artifacts.
...
`corpus_paths` must stay inside `cases/{project}/`.
