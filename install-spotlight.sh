...
# Link the engine-resolved Spotlight skill set into <dest>. skills.manifest
# (generated by `bsig skills resolve`, the engine catalog being the source of
...
if [ "$SPOTLIGHT_VAULT_APP" = "tolaria" ]; then
  ...
  step "Obsidian vault"
  if [ -f "/mnt/c/Program Files/Obsidian/Obsidian.exe" ]; then
  printf "%s✓%s Obsidian installed (Windows)\n" "$_c_green" "$_c_reset"
  else
...
if [ "$SPOTLIGHT_MODE" = "local" ]; then
  ...
  if [ "$SPOTLIGHT_AGENT" = "pi" ]; then
    ...
    if [ "$DRY_RUN" = "1" ]; then
      ...
      link_spotlight_skills "$HOME/.pi/agent/skills/spotlight"
    ...
    if [ "$DRY_RUN" = "1" ]; then
      ...
      link_spotlight_skills "$HOME/.config/opencode/skills/spotlight"
