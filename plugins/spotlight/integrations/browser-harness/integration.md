...
Every Browser Harness acquisition should update `{CASE_DIR}/data/evidence-bundle.json` with:
...
Store screenshots and downloads under `{CASE_DIR}/evidence/`.
...
```text
write-file("{CASE_DIR}/research/browser-harness-task.json", <serialized task JSON>)
execute-shell("browser-harness run --task-file {CASE_DIR}/research/browser-harness-task.json --output {CASE_DIR}/research/browser-harness-result.json")
```
