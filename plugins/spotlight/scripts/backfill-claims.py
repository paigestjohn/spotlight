...
def build_claim_note(project: str, fields: dict, verdict: str, layer: str, date: str) -> tuple[str, str]:
...
def main() -> int:
    ...
    if lock.exists():
        ...
        return 1
    if not dry:
        lock.write_text(f"backfill-claims {__import__('datetime').date.today().isoformat()}", encoding="utf-8")
    ...
    try:
        ...
        for inv in sorted(registry_items(inv_registry, "investigations"), key=lambda i: i["id"]):
            ...
            date = str(fm.get("date", inv.get("date", "")))
            counts_all_verified = (
                str(fm.get("verified_count", "x")) == str(fm.get("total_findings", "y"))
                and "verified_count" in fm
            )
            ...
            for number, title, lines in findings:
                ...
                claim_id, note = build_claim_note(project, fields, verdict, layer, date)
                notes[claim_id] = note
                claims.append({
                    "id": claim_id,
                    "project": project,
        ...
        if existing_aliases.exists():
            try:
                generated = json.loads(existing_aliases.read_text(encoding="utf-8")).get("generated", "backfill")
            except json.JSONDecodeError:
                pass
        alias_doc = {"schema_version": "1.0", "generated": generated,
        ...
        if proposals_path.exists():
            try:
                ...
                pass
        known_pairs = {tuple(p.get("entities", [])) for p in proposals_doc.get("proposals", [])}
        date_anchor = max((str(c.get("recorded", "")) for c in claims), default="") or "backfill"
        for collision in collisions:
            pair = tuple(collision["entities"])
